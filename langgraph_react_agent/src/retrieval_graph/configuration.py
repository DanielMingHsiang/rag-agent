...
class IndexConfiguration:
    """
    indexing 和 retrive 資料的 configuration。
    此類定義配置 indexing 所需的參數和 retrive 過程，包括使用者辨識、embedding 模型選擇、retriver provider 選擇、和 query parameter。
    """
...
class Configuration(BaseConfiguration):
    ...
    query_system_prompt: str = field(
        ...
        metadata={"description": "用於產生最佳化查詢的 的 system prompt"},
    )

    response_system_prompt: str = field(
        default=prompts.RESPONSE_SYSTEM_PROMPT,
        metadata={"description": "用於產生 response 的 system prompt"},
    )
