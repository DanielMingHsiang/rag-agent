...
# 測試評估
- 在檢索條件加上 rerank 機制，可以再次篩選更高的相關資訊，但會較慢
- 調整檢索取出的分塊上限，與精準度有關
    1. 較高數量的分塊，資訊涵蓋多，有可能更精準
    2. 但因此測試集資料乾淨，只與 EC2 相關，因此可使用較大分塊測試
    3. 若資料集較大，涵蓋太多「只是提及 EC2」，但實際上不相關的資料，會降低檢索精度（因為分塊的 ranking 可能把上述情形排行到前面）
    4. 一次檢索的分塊調整太大，資訊過大或雜訊太多，LLM 容易混淆或回應速度太慢
