...
- 調整檢索取出的分塊上限，與精準度有關
    > 1. 此測試集資料乾淨，只與 EC2 相關，因此可使用較大分塊測試，提高資訊量與精準度
    > 2. 若資料集大，涵蓋太多「只是提及 EC2」，但實際上不相關的資料，會降低檢索精度（因為分塊的 ranking 可能把上述情形排行到前面）
    > 3. 若資料集大，一次檢索過多分塊，會造成資訊量過大＆雜訊多，LLM 容易混淆或回應速度慢
